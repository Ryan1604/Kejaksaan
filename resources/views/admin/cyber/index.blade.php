    ...
    <script>
        $(document).ready(function() {
            ...
            $('#btn-save').click(function() {
                ...
                var type = "POST";
                var ajaxurl = '{{ route('admin.cyber.store') }}';
                ...
                if (state == "update") {
                    ...
                    var id = $('#id').val();
                    type = "PUT";
                    ajaxurl = '{{ route('admin.cyber.store') }}' + '/' + id;
                }
            ...
            $('body').on('click', '#btn-edit', function() {
                var id = $(this).val();
                $.get('{{ route('admin.cyber.index') }}' + '/' + id + '/edit', function(data) {
                    $('#company-form').find('.form-control').removeClass('is-invalid is-valid');
                    $('#id').val(data.id);
                    $('#name').val(data.name);
                ...
                swal("Peringatan!", "Apakah anda yakin?", "warning", {
                    ...
                    switch (value) {
                        case "ok" :
                            $.ajax({
                                type: "DELETE",
                                url: '{{ route('admin.cyber.store') }}' + '/' + id,
                                success: function(data) {
                                    $('#company-table').DataTable().draw(false);
                                    $('#company-table').DataTable().on('draw', function() {
                                        $('[data-toggle="tooltip"]').tooltip();
