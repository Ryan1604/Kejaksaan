...
@section('title', 'Infrastruktur Bandara')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#company-table').DataTable({
                processing: true,
                serverSide: true,
                ajax: '{{ route('admin.bandara.index') }}',
                columns: [
                    {
                        data: 'DT_RowIndex',
                        name: 'DT_RowIndex',
            ...
            $('#btn-save').click(function() {
                ...
                var type = "POST";
                var ajaxurl = '{{ route('admin.bandara.store') }}';
                ...
                if (state == "update") {
                    ...
                    var id = $('#id').val();
                    type = "PUT";
                    ajaxurl = '{{ route('admin.bandara.store') }}' + '/' + id;
                }
            ...
            $('body').on('click', '#btn-edit', function() {
                var id = $(this).val();
                $.get('{{ route('admin.bandara.index') }}' + '/' + id + '/edit', function(data) {
                    $('#company-form').find('.form-control').removeClass('is-invalid is-valid');
                    $('#id').val(data.id);
                    $('#name').val(data.name);
                ...
                swal("Whoops!", "Are you sure want to delete?", "warning", {
                    ...
                    switch (value) {
                        case "ok" :
                            $.ajax({
                                type: "DELETE",
                                url: '{{ route('admin.bandara.store') }}' + '/' + id,
                                success: function(data) {
                                    $('#company-table').DataTable().draw(false);
                                    $('#company-table').DataTable().on('draw', function() {
                                        $('[data-toggle="tooltip"]').tooltip();
