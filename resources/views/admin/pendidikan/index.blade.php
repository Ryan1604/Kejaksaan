                ...
                <div class="modal-body">
                    <form id="company-form">
                        ...
                        <div class="form-group">
                            <label for="name">Pendidikan <sup class="text-danger">*</sup></label>
                            <input type="text" class="form-control" id="name" name="name"
                                placeholder="Masukkan pendidikan..." autocomplete="off">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#btn-save').click(function() {
                ...
                var type = "POST";
                var ajaxurl = '{{ route('admin.pendidikan.store') }}';
                ...
                if (state == "update") {
                    ...
                    var id = $('#id').val();
                    type = "PUT";
                    ajaxurl = '{{ route('admin.pendidikan.store') }}' + '/' + id;
                }
            ...
            $('body').on('click', '#btn-edit', function() {
                ...
                $.get('{{ route('admin.pendidikan.index') }}' + '/' + id + '/edit', function(data) {
                    $('#company-form').find('.form-control').removeClass('is-invalid is-valid');
                    $('#id').val(data.id);
                    $('#name').val(data.name);
                ...
                swal("Whoops!", "Are you sure want to delete?", "warning", {
                    ...
                    switch (value) {
                        case "ok" :
                            $.ajax({
                                type: "DELETE",
                                url: '{{ route('admin.pendidikan.store') }}' + '/' + id,
                                success: function(data) {
                                    $('#company-table').DataTable().draw(false);
                                    $('#company-table').DataTable().on('draw', function() {
                                        $('[data-toggle="tooltip"]').tooltip();
