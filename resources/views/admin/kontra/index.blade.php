...
@section('title', 'Kontra Penginderaan')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#btn-save').click(function() {
                ...
                var type = "POST";
                var ajaxurl = '{{ route('admin.kontra.store') }}';
                ...
                if (state == "update") {
                    ...
                    var id = $('#id').val();
                    type = "PUT";
                    ajaxurl = '{{ route('admin.kontra.store') }}' + '/' + id;
                }
            ...
            $('body').on('click', '#btn-edit', function() {
                var id = $(this).val();
                $.get('{{ route('admin.kontra.index') }}' + '/' + id + '/edit', function(data) {
                    $('#company-form').find('.form-control').removeClass('is-invalid is-valid');
                    $('#id').val(data.id);
                    $('#name').val(data.name);
                ...
                swal("Whoops!", "Are you sure want to delete?", "warning", {
                    ...
                    switch (value) {
                        case "ok" :
                            $.ajax({
                                type: "DELETE",
                                url: '{{ route('admin.kontra.store') }}' + '/' + id,
                                success: function(data) {
                                    $('#company-table').DataTable().draw(false);
                                    $('#company-table').DataTable().on('draw', function() {
                                        $('[data-toggle="tooltip"]').tooltip();
