...
class AgrariaController extends Controller
{
    ...
    {
        if (request()->ajax()) {
            return datatables()->of(Agraria::with('kecamatan', 'biodata')
                ->orderBy('updated_at', 'DESC')
                ->get())
                ->addColumn('biodata', 'admin.agraria.biodata')
                ->addColumn('kecamatan', 'admin.agraria.kecamatan')
                ->addColumn('action', 'admin.agraria.action')
        ...
        return view('admin.agraria.index', compact('kecamatan', 'biodata'));
    }
