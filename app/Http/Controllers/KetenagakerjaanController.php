...
class KetenagakerjaanController extends Controller
{
    ...
    {
        if (request()->ajax()) {
            return datatables()->of(Ketenagakerjaan::with('kecamatan', 'biodata')
                ->orderBy('updated_at', 'DESC')
                ->get())
                ->addColumn('biodata', 'admin.ketenagakerjaan.biodata')
                ->addColumn('kecamatan', 'admin.ketenagakerjaan.kecamatan')
                ->addColumn('action', 'admin.ketenagakerjaan.action')
        ...
        return view('admin.ketenagakerjaan.index', compact('kecamatan', 'biodata'));
    }
    ...
    {
        $data = Ketenagakerjaan::findOrFail($id);
    ...
    {
        $data = Ketenagakerjaan::destroy($id);
