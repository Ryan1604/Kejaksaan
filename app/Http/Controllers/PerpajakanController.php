...
class PerpajakanController extends Controller
{
    ...
    {
        if (request()->ajax()) {
            return datatables()->of(Perpajakan::with('kecamatan', 'biodata')
                ->orderBy('updated_at', 'DESC')
                ->get())
                ->addColumn('biodata', 'admin.perpajakan.biodata')
                ->addColumn('kecamatan', 'admin.perpajakan.kecamatan')
                ->addColumn('action', 'admin.perpajakan.action')
        ...
        return view('admin.perpajakan.index', compact('kecamatan', 'biodata'));
    }
