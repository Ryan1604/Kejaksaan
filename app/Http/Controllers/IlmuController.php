...
class IlmuController extends Controller
{
    ...
    {
        if (request()->ajax()) {
            return datatables()->of(Ilmu::with('kecamatan', 'biodata')
                ->orderBy('updated_at', 'DESC')
                ->get())
                ->addColumn('biodata', 'admin.ilmu.biodata')
                ->addColumn('kecamatan', 'admin.ilmu.kecamatan')
                ->addColumn('action', 'admin.ilmu.action')
        ...
        return view('admin.ilmu.index', compact('kecamatan', 'biodata'));
    }
