...
class ListrikController extends Controller
{
    ...
    {
        if (request()->ajax()) {
            return datatables()->of(Listrik::with('kecamatan', 'biodata')
                ->orderBy('updated_at', 'DESC')
                ->get())
                ->addColumn('biodata', 'admin.listrik.biodata')
                ->addColumn('kecamatan', 'admin.listrik.kecamatan')
                ->addColumn('action', 'admin.listrik.action')
        ...
        return view('admin.listrik.index', compact('kecamatan', 'biodata'));
    }
