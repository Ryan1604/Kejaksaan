...
class TanggulController extends Controller
{
    ...
    {
        if (request()->ajax()) {
            return datatables()->of(Tanggul::with('kecamatan', 'biodata')
                ->orderBy('updated_at', 'DESC')
                ->get())
                ->addColumn('biodata', 'admin.tanggul.biodata')
                ->addColumn('kecamatan', 'admin.tanggul.kecamatan')
                ->addColumn('action', 'admin.tanggul.action')
        ...
        return view('admin.tanggul.index', compact('kecamatan', 'biodata'));
    }
