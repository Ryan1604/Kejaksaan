...
class PertanianController extends Controller
{
    ...
    {
        if (request()->ajax()) {
            return datatables()->of(Pertanian::with('kecamatan', 'biodata')
                ->orderBy('updated_at', 'DESC')
                ->get())
                ->addColumn('biodata', 'admin.pertanian.biodata')
                ->addColumn('kecamatan', 'admin.pertanian.kecamatan')
                ->addColumn('action', 'admin.pertanian.action')
        ...
        return view('admin.pertanian.index', compact('kecamatan', 'biodata'));
    }
