...
class PenyelenggaraanController extends Controller
{
    ...
    {
        if (request()->ajax()) {
            return datatables()->of(Penyelenggaraan::with('kecamatan', 'biodata')
                ->orderBy('updated_at', 'DESC')
                ->get())
                ->addColumn('biodata', 'admin.penyelenggaraan.biodata')
                ->addColumn('kecamatan', 'admin.penyelenggaraan.kecamatan')
                ->addColumn('action', 'admin.penyelenggaraan.action')
        ...
        return view('admin.penyelenggaraan.index', compact('kecamatan', 'biodata'));
    }
    ...
    {
        $data = Penyelenggaraan::findOrFail($id);
    ...
    {
        $data = Penyelenggaraan::destroy($id);
