...
class PenodaanController extends Controller
{
    ...
    {
        if (request()->ajax()) {
            return datatables()->of(Penodaan::with('kecamatan', 'biodata')
                ->orderBy('updated_at', 'DESC')
                ->get())
                ->addColumn('biodata', 'admin.penodaan.biodata')
                ->addColumn('kecamatan', 'admin.penodaan.kecamatan')
                ->addColumn('action', 'admin.penodaan.action')
        ...
        return view('admin.penodaan.index', compact('kecamatan', 'biodata'));
    }
