...
class PancasilaController extends Controller
{
    ...
    {
        if (request()->ajax()) {
            return datatables()->of(Pancasila::with('kecamatan', 'biodata')
                ->orderBy('updated_at', 'DESC')
                ->get())
                ->addColumn('biodata', 'admin.pancasila.biodata')
                ->addColumn('kecamatan', 'admin.pancasila.kecamatan')
                ->addColumn('action', 'admin.pancasila.action')
        ...
        return view('admin.pancasila.index', compact('kecamatan', 'biodata'));
    }
