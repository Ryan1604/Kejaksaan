...
class SeparatisController extends Controller
{
    ...
    {
        if (request()->ajax()) {
            return datatables()->of(Gerakan::with('kecamatan', 'biodata')
                ->orderBy('updated_at', 'DESC')
                ->get())
                ->addColumn('biodata', 'admin.separatis.biodata')
                ->addColumn('kecamatan', 'admin.separatis.kecamatan')
                ->addColumn('action', 'admin.separatis.action')
        ...
        return view('admin.separatis.index', compact('kecamatan', 'biodata'));
    }
