...
class MoneterController extends Controller
{
    ...
    {
        if (request()->ajax()) {
            return datatables()->of(Moneter::with('kecamatan', 'biodata')
                ->orderBy('updated_at', 'DESC')
                ->get())
                ->addColumn('biodata', 'admin.moneter.biodata')
                ->addColumn('kecamatan', 'admin.moneter.kecamatan')
                ->addColumn('action', 'admin.moneter.action')
        ...
        return view('admin.moneter.index', compact('kecamatan', 'biodata'));
    }
